...
<div class="kpi-grid">
    ...
    @include('admin.analytics.components._kpi-card', [
        ...
        'value' => $kpis['total_exam_results'] > 0 ? $kpis['overall_pass_rate'] . '%' : 'No data',
    ...
    @php
        ...
        $bestDeptHasData = $bestDept && (($bestDept['total_results'] ?? 0) > 0);
    @endphp
    @include('admin.analytics.components._kpi-card', [
        'label' => 'Highest Performing Department',
        'value' => $bestDeptHasData ? $bestDept['pass_rate'] . '%' : 'No data',
        'unit' => $bestDeptHasData ? $bestDept['name'] : 'No data',
    ...
    @php
        ...
        $worstDeptHasData = $worstDept && (($worstDept['total_results'] ?? 0) > 0);
    @endphp
    @include('admin.analytics.components._kpi-card', [
        'label' => 'Lowest Performing Department',
        'value' => $worstDeptHasData ? $worstDept['pass_rate'] . '%' : 'No data',
        'unit' => $worstDeptHasData ? $worstDept['name'] : 'No data',
        'icon' => '<svg viewBox="0 0 24 24" fill="currentColor" style="width: 20px; height: 20px;"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>',
    ])
    ...
    @php
        ...
        $riskSubjectHasData = $riskSubject && (($riskSubject['total_results'] ?? 0) > 0);
    @endphp
    @include('admin.analytics.components._kpi-card', [
        'label' => 'Highest Risk Subject',
        'value' => $riskSubjectHasData ? $riskSubject['pass_rate'] . '%' : 'No data',
        'unit' => $riskSubjectHasData ? $riskSubject['name'] : 'No data',
    ...
    @php
        ...
        $riskTeacherHasData = $riskTeacher && (($riskTeacher['total_results'] ?? 0) > 0);
    @endphp
    @include('admin.analytics.components._kpi-card', [
        'label' => 'Highest Risk Teacher',
        'value' => $riskTeacherHasData ? $riskTeacher['pass_rate'] . '%' : 'No data',
        'unit' => $riskTeacherHasData ? $riskTeacher['name'] : 'No data',
