...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar-nav { flex: 1; padding: 16px 0; overflow-y: auto; position: relative; z-index: 1; overflow-touch; }
        .nav-section-label {
...
>
    ...
    <nav class="sidebar-nav overflow-touch">
        <div class="nav-section-label">Overview</div>
        <a href="{{ route('admin.dashboard') }}" class="nav-item {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>
            Dashboard
        </a>

        <div class="nav-section-label">Management</div>
        <a href="{{ route('admin.school-years.index') }}" class="nav-item {{ request()->routeIs('admin.school-years*') ? 'active' : '' }}">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            School Years
        </a>
...
<script>
    ...
    function handleSwipe() {
        ...
        // Get reference to Alpine component (body element with x-data)
        const body = document.body;
        const alpineComponent = body.__x; // Alpine stores component data here

        // Swipe from left to right (open sidebar)
        if (diffX > threshold && touchStartX < 50) {
            if (window.innerWidth < 640) {
                if (alpineComponent && alpineComponent.$data) {
                    alpineComponent.$data.sidebarOpen = true;
                }
            }
        }
        // Swipe from right to left (close sidebar)
            ...
            if (window.innerWidth < 640) {
                if (alpineComponent && alpineComponent.$data) {
                    alpineComponent.$data.sidebarOpen = false;
                }
            }
        }
    }
